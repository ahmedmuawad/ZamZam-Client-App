...
class NotificationHelper {
      ...
      try{
        if(payload!.isNotEmpty) {
          MyApp.navigatorKey.currentState?.push(
              MaterialPageRoute(builder: (context) => OrderDetailsScreen(orderModel: OrderModel(), orderId: int.parse(payload))));
        }
      }catch (e) {}
      return;
    });
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      ...
      try{
        if(message.notification?.titleLocKey != null && message.notification!.titleLocKey!.isNotEmpty) {
          MyApp.navigatorKey.currentState!.push(
              MaterialPageRoute(builder: (context) => OrderDetailsScreen(orderModel: OrderModel(), orderId: int.parse(message.notification!.titleLocKey!))));
        }
      }catch (e) {}
    });
  }
  ...
  static Future<void> showBigTextNotification(String? title, String? body, String? orderID, FlutterLocalNotificationsPlugin fln) async {
    ...
    AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
      'your channel id', 'your channel name', playSound: true,
      styleInformation: bigTextStyleInformation, priority: Priority.max, importance: Importance.max,
